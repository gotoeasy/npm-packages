...
module.exports = bus.on('样式库', function(rs={}){
...
module.exports = bus.on('样式库引用', function(){
    
    return async function (pkg, ...classnames){
       let csslib = await bus.at('样式库', pkg);
       let ary = [];
       classnames.forEach(cls => ary.push(cls.startsWith('.') ? cls : ('.' + cls)));
       let css = csslib.get( ...ary );
// console.info(MODULE, '---------------csslib---------------', pkg, classnames, css)
       return css;
    }
